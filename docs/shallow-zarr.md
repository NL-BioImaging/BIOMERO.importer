...
BIOMERO.shallower provides the shared normalization implementation. The
importer runs that library locally or validates results normalized on Slurm.
...
## Choose where normalization runs

| Route | Normalization | Importer responsibility |
| --- | --- | --- |
| Local | Importer runs the shared shallower library after preprocessing | Compare identities, normalize, resolve sources and register results |
...
Within enabled shallow workflows, remote normalization is preferred. To use
the local path, set `BIOMERO_REMOTE_SHALLOW_ZARR=false` on both the workflow
worker and importer. This avoids the additional Slurm normalization job but
transfers full results and performs normalization on the importer host.
`BIOMERO_SHALLOW_ZARR_WORKERS` controls local identity parallelism; choose it
for the host's CPU and storage capacity.

For remote normalization, enable `BIOMERO_REMOTE_SHALLOW_ZARR` and configure
...
## Import remotely normalized results

The workflow's import order additionally supplies `remoteReceipts` from its
completed normalizer task. The importer validates the report checksum,
...
pixel hashing and normalization. Local source resolution and OMERO
...
can use local normalization. A corrupt or incomplete shallow result is not
...
before registration. Local normalization still verifies returned pixels.

The importer records the actual storage outcome in
`.biomero-import-storage.json` beside the result. Local normalization records
